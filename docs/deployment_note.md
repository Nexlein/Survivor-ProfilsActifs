...
Conformément aux exigences du Ministère (Thomas Vignal - Conseiller Numérique), cette note décrit la stratégie de déploiement souverain de JibJob.

## 1. Souveraineté & Hébergement
L'application JibJob est conçue pour fonctionner **intégralement en local** ou sur un serveur privé (on-premise). 
...
pm2 start npm --name "jibjob-api" -- start
```
